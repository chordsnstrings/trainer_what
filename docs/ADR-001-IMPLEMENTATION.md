...
Supply one container image for web/API/worker plus separate migrations. A single-host Compose topology is a reproducible starting point for an approved DigitalOcean environment, not a claim that the source's eventual DOKS/managed services architecture has been provisioned. Migration credentials are absent from runtime services; provider credentials are absent from the web service. Region, residency, hosting budget, backups and availability design remain release decisions.
