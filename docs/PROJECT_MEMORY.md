...
| Current request | Build the complete nutrition integration. Case-based coach onboarding teaches recommendations; routine nutrition delivery is automatic, with human handling of exceptions. |
...
- Production needs NUTRITION_ENABLED, qualified scope review (NUTRITION_SCOPE_APPROVED), actual coach/food material and successful configured-model evaluation; worker scheduling needs PostgreSQL. Existing provider, infrastructure and legal gates remain. Optional barcode/food-database/photo adapters (043) remain disabled because no provider/account/rights/storage contract was selected; the complete core meal/recipe/grocery flow uses versioned coach food facts.
