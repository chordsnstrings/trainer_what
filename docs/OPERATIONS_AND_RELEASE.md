...
| DigitalOcean | Owner authorized necessary new project/instance/resources on 25 September; existing resources must remain untouched. Verify actual access, region/size/pricing and restore capability; direct connectivity fails before authentication and installed-plugin provisioning commands are pending exposure in the active session | Deployment preparation and tests; use DIGITALOCEAN_DEPLOYMENT.md and create-response ownership IDs; customer-data placement remains separately reviewed |
