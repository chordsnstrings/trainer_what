...
**Active application work — 25 September 2026:** deployment is stopped; Claude will handle it. The GymMembership setup workflow is manual-only and must not be launched during this slice. Superadmin encrypted configuration, provider runtime wiring, trainer Design Studio and required meal-photo/barcode logging are implemented and undergoing combined verification. Every new/changed view must have actual desktop/mobile screenshots. The owner authorized bounded parallel implementation agents and prohibits cloud-browser use. See SUPERADMIN_AND_CUSTOMISATION.md and BUILD_STATUS.md for behavior and observed evidence.
