...
**Infrastructure authorization — 25 September 2026, 12:35 Asia/Dubai:** the owner explicitly authorized a new DigitalOcean project, instance and supporting resources, while prohibiting changes to existing resources. The [deployment boundary](DIGITALOCEAN_DEPLOYMENT.md) governs this work. At 13:11, authenticated DigitalOcean app account/region/size reads succeeded. Project/VPC/firewall and host-configuration operations remain unavailable; direct shell HTTPS fails and the browser project page is unavailable. No cloud resources are created. The owner also requires automatic deployment of tested Git updates; the deployment workflow is pending. Do not repeat authorization, installation or the already-enabled personal network setting requests.
