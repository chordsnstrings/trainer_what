...
**Active infrastructure work — 25 September 2026:** the owner authorized a wholly new DigitalOcean project named GymMembership and a new server; existing resources must remain untouched. Dedicated VPC and cloud firewall are deferred. Work 005 is IN_PROGRESS: the setup workflow, durable ownership checkpoints and host updater for checked main commits are published at 224b893 and CI-verified (37 deployment checks and both application jobs passed). Live verification remains pending; see VERIFICATION_2026-09-25_DEPLOYMENT.md. The selected initial footprint is blr1, 2 vCPU/4 GB, capped at USD 24/month compute. No cloud resources or live deployment evidence exist. The remaining setup access step is a repository Actions secret named DO_PROVISION_TOKEN. The owner authorized bounded parallel agents for speed and subsequently required direct DigitalOcean API access and prohibited cloud-browser use; do not repeat that sign-in, infrastructure permission, installation or network-toggle requests. Follow [the deployment guide](DIGITALOCEAN_DEPLOYMENT.md).
...
The owner confirmed **workout only** and higher-priced **workout + nutrition** subscriptions. The [nutrition integration plan](NUTRITION_INTEGRATION_PLAN.md) adds internal work IDs **035–044** for entitlements, case-based coach teaching, intake/consent, recipes/calculations, connected weekly plans/daily meals/cooking/portions/groceries, nutrition Brain/Twin, exception handling and release evidence. Core 035–042 and 044 engineering verification have passed with synthetic provider fixtures; [nutrition evidence](VERIFICATION_2026-09-25_NUTRITION.md) records the exact boundary. Live-model and production qualification remain open. At 12:27 Asia/Dubai on 25 September, the owner made meal-photo logging and food-barcode scanning required scope: 043 is committed but not implemented, and 044 must gain evidence for both new journeys. The original 001–034 baseline remains intact and these IDs are not created GitHub issues. Conditional onboarding learns what the coach recommends for realistic cases and verifies scope/coverage on held-out cases. Qualified routine outputs are delivered automatically; the coach handles exceptions rather than approving every plan. Manual food/recipe workflows are the foundation, not the complete product. The core release includes automatic nutrition delivery; required meal-photo and barcode flows are the next nutrition engineering slice. Subscriber confirmation saves these entries; routine coach approval is not required. Clinical scope and provider rights remain reviewed inputs.
