...
| Staging / DigitalOcean | New isolated project/resources explicitly authorized on 25 September; direct API connectivity timed out before authentication. DigitalOcean installation confirmed, but the running session does not expose its provisioning tools; no resource creation or deployment. See DIGITALOCEAN_DEPLOYMENT.md. |
