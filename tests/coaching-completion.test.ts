...
import { trainingProgramSchema, trainingSchedule } from "../packages/domain/src/coaching-completion.ts";
...
test("dated multiweek programs preserve distinct prescriptions through substitutions, corrections and exercise-only progression", async () => {
  const squat = { name: "Front squat", sets: 3, reps: 8, loadKg: 20, restSeconds: 90, rir: 2, cue: "Controlled technique", alternatives: [{ name: "Split squat", loadKg: 10, cue: "Keep balance" }] };
  const row = { name: "Cable row", sets: 3, reps: 12, loadKg: 30, restSeconds: 60, rir: 3, cue: "Stable torso" };
  const body = { title: "Three-week strength block", goal: "Consistent practice", daysPerWeek: 2, weeks: 3, exercises: [squat], sessions: [{ label: "Lower body", weekday: 1, exercises: [squat] }, { label: "Upper body", weekday: 4, exercises: [row] }] };
  assert.equal(trainingSchedule(trainingProgramSchema.parse(body), "2026-09-28", "Europe/London").length, 6);
  const created = await request("/programs", "POST", { subscriberId: client.userId, program: body, startDate: "2026-09-28", timezone: "Asia/Dubai" }, coach);
  assert.equal(created.statusCode, 200, created.body); const p = created.json();
  const view = (await request("/training/overview", "GET", undefined, client)).json();
  const planned = view.records.filter((r: any) => r.kind === "planned_session" && r.data.programId === p.id).sort((a: any, b: any) => a.data.date.localeCompare(b.data.date));
  assert.equal(planned.length, 6); assert.equal(planned[0].data.date, "2026-09-28"); assert.equal(planned[1].data.program.exercises[0].name, "Cable row");
  const started = await request("/workouts/start", "POST", { programId: p.id, plannedSessionId: planned[0].id }, client); assert.equal(started.statusCode, 200, started.body); const workout = started.json();
  assert.equal((await request(`/workouts/${workout.id}/substitute`, "POST", { version: workout.version, exercise: "Front squat", replacement: "Unapproved exercise", reason: "equipment_unavailable" }, client)).statusCode, 400);
  const swapped = await request(`/workouts/${workout.id}/substitute`, "POST", { version: workout.version, exercise: "Front squat", replacement: "Split squat", reason: "equipment_unavailable" }, client); assert.equal(swapped.statusCode, 200, swapped.body);
  const logged = await request(`/workouts/${workout.id}/sets`, "POST", { eventKey: randomUUID(), exercise: "Split squat", set: 1, reps: 8, loadKg: 10, rir: 2, notes: "Smooth reps" }, client); assert.equal(logged.statusCode, 200, logged.body);
  const correctionBody = { revision: 0, reps: 9, loadKg: 12, rir: 3, note: "Correcting my typed weight" };
  const corrected = await request(`/workouts/${workout.id}/sets/${logged.json().id}/correct`, "POST", correctionBody, client); assert.equal(corrected.statusCode, 200, corrected.body);
  assert.equal((await request(`/workouts/${workout.id}/sets/${logged.json().id}/correct`, "POST", correctionBody, client)).statusCode, 409);
  assert.equal((await request(`/workouts/${workout.id}/sets/${logged.json().id}/correct`, "POST", { ...correctionBody, revision: 1 }, other)).statusCode, 404);
  const original = await db.tenant(client, (tx) => tx.query("SELECT * FROM workout_events WHERE id=$1", [logged.json().id])); assert.equal(original[0].data.loadKg, 10, "Original evidence remains intact");
  const revised = await request(`/programs/${p.id}/progression`, "POST", { version: p.version, exercise: "Front squat", loadKg: 22, reps: 8, rir: 2, note: "Coach reviewed squat progression for the next session" }, coach); assert.equal(revised.statusCode, 200, revised.body);
  const updated = (await request("/training/overview", "GET", undefined, client)).json();
  assert.equal(updated.sets.find((s: any) => s.id === logged.json().id).data.loadKg, 12);
  const future = updated.records.filter((r: any) => r.kind === "planned_session" && r.data.programId === revised.json().id);
  assert.equal(future.length, 5); assert.ok(future.filter((r: any) => r.data.label === "Upper body").every((r: any) => r.data.program.exercises[0].loadKg === 30), "Progressing one exercise must not change another day");
  assert.equal(updated.records.find((r: any) => r.id === workout.id).data.program.exercises[0].name, "Split squat", "Active workout keeps its own snapshot");
  const twin = await request(`/clients/${client.userId}/twin`, "GET", undefined, client); assert.equal(twin.statusCode, 200, twin.body);
  const performance = twin.json().data.coaching.training.performance.find((r: any) => r.exercise === "Split squat"); assert.equal(performance.volumeKg, 108); assert.deepEqual(performance.sourceCorrectionIds, [corrected.json().id]);
  assert.equal((await request(`/workouts/${workout.id}/finish`, "POST", {}, client)).statusCode, 200);
});
test("template assignment, dated rescheduling and chat history enforce membership and stale-edit guards", async () => {
  const created = await request("/programs", "POST", { program: { title: "Reusable practice", goal: "Two-week habit", daysPerWeek: 1, weeks: 2, exercises: [{ name: "Step up", sets: 2, reps: 8, loadKg: 8, restSeconds: 60, rir: 2, cue: "Control the lowering" }] } }, coach); assert.equal(created.statusCode, 200, created.body); const p = created.json();
  const body = { version: p.version, subscriberId: client.userId, startDate: "2098-01-06", timezone: "Europe/London" };
  assert.equal((await request(`/programs/${p.id}/assign`, "POST", body, other)).statusCode, 404);
  const assigned = await request(`/programs/${p.id}/assign`, "POST", body, coach); assert.equal(assigned.statusCode, 200, assigned.body);
  const overview = (await request("/training/overview", "GET", undefined, client)).json(), session = overview.records.find((r: any) => r.kind === "planned_session" && r.data.programId === assigned.json().id);
  const moved = await request(`/training/sessions/${session.id}/reschedule`, "POST", { version: session.version, date: "2098-02-01", note: "Travel week changed" }, client); assert.equal(moved.statusCode, 200, moved.body);
  assert.equal((await request(`/training/sessions/${session.id}/cancel`, "POST", { version: session.version, note: "Stale cancel attempt" }, client)).statusCode, 409);
  assert.equal((await request(`/training/sessions/${session.id}/cancel`, "POST", { version: session.version + 1, note: "Skip this planned workout" }, client)).statusCode, 200);
  const message = await request("/messages", "POST", { text: "I moved my planned workout", subscriberId: client.userId }, client); assert.equal(message.statusCode, 200, message.body);
  const thread = await request(`/messages/thread?subscriberId=${client.userId}`, "GET", undefined, coach); assert.equal(thread.statusCode, 200, thread.body); assert.ok(thread.json().messages.find((m: any) => m.id === message.json().id));
  assert.equal((await request(`/messages/thread?subscriberId=${client.userId}`, "GET", undefined, other)).statusCode, 404);
});
