...
**Owner clarification that supersedes the attachment:** Stripe must handle **both subscriber collection and trainer payouts to UAE IBANs** using the operator's existing Stripe account and supplied API access. The attachment instead says Stripe handles collection while a separate operator-supplied `PayoutProvider` pays trainers. Treat this plan's Stripe direction as the current instruction. Before implementing the financial architecture, validate the exact Stripe account capabilities, UAE connected-account eligibility and requirements, and whether Connect transfers plus connected-account payouts meet the intended monthly reconciliation flow. An IBAN alone is not proof of payout eligibility. If the account cannot pay a category of trainers, do not claim that category can receive Stripe payouts; leave their financial status pending and escalate the constraint for a business decision. Keep a provider boundary so the implementation can change without rebuilding the ledger. See Phase 0 and Decision D-01.
...
- Stripe collection **and Stripe-based connected-account payout**, monthly reconciliation, immutable financial ledger, marginal commission, AI/voice cost accounting and trainer/admin finance views.
...
2. The user's later explicit Stripe instruction overrides the attachment's separate payout-rail language in the opening table, §§3.2, 10, 16–17 and Appendices A/C/F/G/I/J/L/M/N. Update the master spec when an editable canonical version is established. The actual Stripe integration mode is an early empirical decision, not an assumption.
...
packages/providers         Stripe, model, wearable, voice, email, domain, storage
...
**Financial records:** use integer AED minor units, an immutable balanced ledger or equivalent auditable postings, source object references and compensating entries. Separate payment state, customer entitlements, trainer liability, platform revenue and actual bank payout state. A Stripe event is evidence of external state; the reconciled internal ledger is the platform's accounting record. Record fee schedule and allocation algorithm versions for every charge and adjustment. Finance sign-off must settle the mixed-price marginal-band algorithm before live charging (§10.3, §10.11).
...
| 4 — Stripe commerce and paid pilot | 4–8 weeks, starts with Phase 0 | Checkout/renewal, Stripe Connect onboarding and IBAN, ledger, commission, refunds/cancellation, monthly transfer/payout, reconciliation, finance views | Full sandbox cycle and approval; tightly controlled live canary only after finance/legal/security gates |
...
**P0-01 Stripe feasibility, owner: finance/backend.** With the supplied Stripe account in test mode, inspect enabled Connect capabilities, supported UAE legal entity types and verification fields, payouts to UAE external bank accounts, platform/connected account responsibility, charge topology, transfers and payout timing. Create one representative account each for the licensed-company and individual/unlicensed cases if Stripe permits; collect required KYC/beneficial owner/business details through Stripe-hosted/embedded onboarding where possible. Test a subscription charge, settlement, monthly allocation to a connected account, payout to a UAE IBAN, webhooks, a failed/returned payout, refund after transfer and a dispute. Record object IDs, state transitions, fees and where balances can legally/technically be held. Choose **destination charges or separate charges/transfers** according to these tests and counsel/accountant review. Do not hardcode Custom account capability or a one-field IBAN flow. If Stripe cannot support a trainer category, signup/Brain creation may continue, but selling and payouts for that category require a documented policy and cannot silently use an unapproved bank rail. Stripe Global Payouts is an alternative only if the *actual* account is enabled as a supported sender; public documentation describing UAE recipients from other sender countries does not prove UAE sender access.

**P0-02 Legal, privacy and tax, owner: counsel/accountant/product.** Draft merchant-of-record and trainer-agent roles, UAE VAT/invoicing, licensing and insurance policy, cancellation/refunds, data processing/cross-border transfers and health-data residency. The specification deliberately defers licence upload as a product signup gate; Stripe's verified KYC/payout requirements and law can still gate live selling or payout. Produce a written decision table by trainer type and financial state. Prepare counsel-ready Subscriber Terms, Trainer Agreement, assumption-of-risk, AI disclosure, Privacy Policy, wearable consent and voice consent; publish only reviewed versions (§14, Appendix E). Absolute injury immunity and hidden sensitive-data/model use are not viable product assumptions.
...
**F-01 Repo and delivery:** README, workspace/build scripts, code owners, lint/type/test jobs, dependency/secret/SBOM scans, migration validation, preview/staging environments, immutable image build, canary/rollback. Separate staging and production DB, secrets, Stripe accounts/modes, webhooks and buckets. Provision with IaC; document costs and state location.
...
### Phase 4 — Stripe billing, finance and payout

**M-01 Checkout/entitlements:** platform Stripe product/price and recurring Checkout/Billing flow with tenant/storefront metadata. Map subscriptions/invoices/charges to internal IDs. Preserve attributions, handle payment failure, renewal, grace, upgrade/downgrade, period-end cancellation and reactivation. Subscriber can cancel renewal without trainer/support permission, keeps access until the displayed period end and gets immediate confirmation; refund is a separate action (§10.4–10.6).

**M-02 Connected accounts and UAE IBAN:** provision Stripe-connected accounts in the tested configuration, use Stripe onboarding or a compliant custom form for account-holder/KYC/external bank details, store Stripe IDs/status and masked IBAN, and show requirements/verification/disabled reasons. Trainer financial status is `not_started → onboarding → requirements_due → verified → restricted/held`, based on actual Stripe states. A trainer who cannot pass required verification cannot be presented as payout ready merely because an IBAN checksum passes.

**M-03 Immutable money flow:** implement minor-unit balanced postings for customer charges, taxes if applicable, processor fees, platform fee, trainer payable, AI/voice/domain charges, refunds/disputes/reserves, Stripe transfer, Stripe payout and returned/failed corrections. Preserve the effective fee schedule and deterministic mixed-price allocation, including the 25%/20%/15%/10% **marginal** subscriber bands (§1.4). Reconcile Stripe balance transactions to internal statements. Test cutoffs, proration, currency, reversals and negative future balances.

**M-04 Monthly settlement:** close an eligible period once; compute payable by trainer after holds/adjustments; execute Stripe transfers and account payouts according to proven balance and connected-account capabilities; reconcile transfer, payout, arrival/failure/return and bank references. A deterministic `(trainer,period,version)` key prevents second transfer or second payout on job/webhook replay. If a payout fails, retain liability and retry only after account remediation. Keep trainer-visible gross-to-net statement and admin exception queue. No live payout until a dry run, negative-path tests and finance sign-off (§10.7, §17.3).

**M-05 Refund/dispute:** specific charge refund request within seven calendar days, trainer approve/decline with notification and audit, exceptional admin override, Stripe refund outcome and offset to current or future payable. A pending request never silently cancels renewal; a cancellation never silently issues a refund. Dispute and chargeback events update reserves and reconciliation. Webhook receipts are signed/verified, stored, deduplicated, acknowledged promptly and handled asynchronously.
...
| --- | --- | --- |
| Stripe account + UAE trainers | Account-specific Connect capability, KYC by trainer type, external IBAN, transfers, payouts, fees, negative/failure cases tested | Continue Brain/product work; keep selling or payout for affected trainer types off |
...
| --- | --- | --- | --- |
| 001 | P0 | Prove existing Stripe account's UAE connected-account, transfer and payout capabilities | API access |
| 002 | P0 | Decide charge topology, funds custody, KYC and merchant-of-record in ADR | 001, counsel |
...
| 019 | P0 | Stripe-connected account onboarding and UAE external IBAN status | 001–002, 011 |
| 020 | P0 | Immutable ledger, marginal fees, AI metering, reconciliation | 018–019 |
| 021 | P0 | Self-service cancellation, refund approval/override and disputes | 018, 020 |
| 022 | P0 | Monthly Stripe transfers/payouts and returned payout recovery | 019–021 |
...
**First working sprint:** resolve 001–004 and author the cross-cutting contracts/ADRs; in parallel create 005, the golden trainer test fixture and initial design primitives. Do not start by generating dozens of UI screens or provisioning a large production cluster before Stripe and residency feasibility are known.

## 7. Pilot and full release acceptance

**Controlled paid pilot:** an approved UAE trainer signs up, teaches and approves a Brain, passes evaluation, configures a Stripe-connected payout account and IBAN, publishes a branded subdomain, creates a recurring product and enrolls a real subscriber. The subscriber understands the automated service, pays, completes intake, receives a validated program, logs workouts, gets truthful first-person guidance, cancels renewal independently, and can request a charge-specific refund for trainer review. The trainer sees exceptions, Brain evidence, charges, fees and expected payout. A monthly dry run and then approved payout match the ledger and Stripe objects exactly. Cross-tenant and red-flag safety tests pass. Counsel signs relevant data, finance and legal gates.
...
**Business gates are measured, not promised:** activated trainer = 10 paying subscribers within 45 days (configurable); initial targets are activated-trainer CAC <= AED 1,000, payback at 10 subscribers <=3 months, core non-voice COGS <= AED 10 per active subscriber/month, and contribution margin >=70% (§1.5). Record acquisition, Brain bootstrap, payment/Connect, model, support, storage, domain and voice costs separately. Do not manufacture positive pilot metrics or conflate the platform's recognized revenue with GMV.

## 8. Decisions and unresolved facts

| ID | Current instruction / recommendation | Evidence needed before irreversible implementation |
| --- | --- | --- |
| D-01 | **Stripe pays all eligible trainers to UAE IBANs**; supersedes separate payout provider in attachment | Existing account's Connect configuration, UAE individual/company eligibility/KYC, funding/transfer/payout timing, cost and failure handling. If 'everyone' is not eligible, disclose exact limitation and obtain a policy decision. |
| D-02 | Platform-side subscriber collection with explicit trainer/platform relationship | UAE counsel and accountant approve merchant-of-record, VAT, invoice and liability wording; Stripe agreement supports chosen charge topology. |
| D-03 | Trainer signup and Brain setup are open before licence upload | Counsel/Stripe determine which commerce/payout actions must wait for documents; policy lives centrally, with a truthful status shown to trainer. |
...
- [Stripe Connect payout account collection](https://docs.stripe.com/connect/payouts-bank-accounts) explains connected-account external bank accounts and verification flows; [connected-account payout schedules](https://docs.stripe.com/connect/manage-payout-schedule) explains manual/automatic control. [Stripe Connect UAE pricing](https://stripe.com/ae/connect/pricing) shows account/payout costs. These pages do **not** prove this particular Stripe account can pay every trainer type.
- [Stripe Global Payouts recipient requirements](https://docs.stripe.com/global-payouts/recipient-requirements) describe UAE recipients from listed sender countries; confirm sender eligibility in the actual account before treating this as an alternative.
