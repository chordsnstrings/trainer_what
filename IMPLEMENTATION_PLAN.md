...
**Nutrition scope update (25 September 2026):** [integration plan](docs/NUTRITION_INTEGRATION_PLAN.md), internal work IDs 035–044. Confirmed direction: workout-only and higher-priced workout + nutrition tiers; case-based coach onboarding teaches diet/calorie decisions; routine in-scope nutrition outputs are automatic, with human handling of exceptions. Nutrition connects daily meals, recipes, portions and cooking options to weekly groceries. Meal-photo logging and food-barcode scanning are required combined-tier scope and now have implemented, fixture-verified journeys. The current release passed 91 tests per CI database job, production build/container checks and 252 screenshots across 94 routes. This verifies the implemented subset, not every nutrition or original product requirement. Consult the [full-app audit](docs/APP_AUDIT_2026-09-25.md), [current verification](docs/VERIFICATION_2026-09-25_SETTINGS_DESIGN_CAPTURE.md) and [build status](docs/BUILD_STATUS.md) for concrete code gaps and remaining provider/operational qualification.
